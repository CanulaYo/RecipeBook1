...
class RegistrationFragment : Fragment() {
    ...
    private fun validateAndRegister() {
        ...
        if (email.isEmpty() || !Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            binding.editTextTextEmailAddress.error = "Enter the correct email address"
            hasErrors = true
        }

        if (password.length < 6) {
            binding.editTextTextPassword.error = "The password must contain at least 6 characters."
            hasErrors = true
        }
